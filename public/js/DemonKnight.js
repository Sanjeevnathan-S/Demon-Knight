...
class Character {
  constructor(x, y, animations, frameWidth, frameHeight,scale=1) {
    ...
    this.y = y;
    this.scale=scale;
    this.direction='d';//d for default direction in sprite
  ...
  draw(ctx) {
    ...
    ctx.save();//saving canvas state before transformations
    ...
    if(this.direction!='d'){
      ...
      ctx.drawImage(
        ...
        -this.x - this.frameWidth * this.scale, this.y, this.frameWidth * this.scale, this.frameHeight * this.scale
      );
    }else{
      ctx.drawImage(
        anim.image, 
    ...
    ctx.restore();//restore to avoid affecting other drawings
  }
}
...
class Player extends Character{
  ...
  movements(){
    ...
    if(keys['d'] && !this.isCrouch){
      ...
      if(this.x<(canvas.width*(1-0.05)-this.frameWidth)){
        if(this.direction=='d')this.x+=this.speed*5;
        else this.x-=this.speed*5;
      }
    }
    ...
    if((keys['Control'] && keys['ArrowRight']) && (this.x<canvas.width - this.frameWidth) && this.isGrounded){
      ...
      this.direction='nd';//opposite direction of sprite
      this.x-=this.speed;
      this.setAnimation('walk');
...
// Define the function to send POST to backend API
function saveGameData(score) {
  ...
  fetch('/api/game/save-score', {
    ...
    body: JSON.stringify(data)  // Sending the game data to save
  })
  .then(res => res.json())
  .then(response => {
    console.log('Response from server', response);
    if(response.success){
      HI=response.highScore;//update HI
      console.log("save successful");
    }else{
      console.log("Failed to Save..damn");
    }
  })
  .catch(err => {
...
function fetchHI(){
  ...
  fetch('/api/game/HI',{
    ...
    console.log('Response from server', response);
    if(response.success){
      HI=response.highScore || 0;
      console.log('HighScore Recieved: ',HI);
    }else{
      console.log('failed to fetch highscore of player');
    }
  }).catch(err=>{
...
walkImage.onload = () => {
  idleImage.onload = () => {
    ...
    const preloader=new PreLoader('/videos/preloader.mp4',()=>{requestAnimationFrame(gameLoop);});
    fetchHI();//update HI value from DB at beginning of game
    ...
    function gameLoop(timestamp) {
      ...
      if(isGame){
        ...
        if(!ply1.dead){
          ...
          if(!enemy.dead){
            ctx.fillStyle='red';
            // Health Bar Width based on current health ratio
            const healthBarWidth = enemy.frameWidth * (enemy.health / enemy.maxHealth);
  
            // If the enemy is facing left (opposite direction), adjust the health bar position
            if (enemy.direction !== 'd') {
            // Adjust health bar position for left-facing enemy
              ctx.fillRect(enemy.x + enemy.frameWidth, enemy.y + enemy.frameHeight / 2, healthBarWidth, 10);
              ctx.strokeRect(enemy.x + enemy.frameWidth, enemy.y + enemy.frameHeight / 2, enemy.frameWidth, 10); // Outline
            } else {
              // Default (right-facing) position for the health bar
              ctx.fillRect(enemy.x, enemy.y + enemy.frameHeight / 2, healthBarWidth, 10);
              ctx.strokeRect(enemy.x, enemy.y + enemy.frameHeight / 2, enemy.frameWidth, 10); // Outline
            }

            // Health text (positioned at the center of the health bar)
            ctx.fillStyle = 'yellow';
            ctx.font = '18px Arial';
  
             // For flipped sprite (left-facing), adjust text position accordingly
            if (enemy.direction !== 'd') {
              // For left-facing enemy, position the health text on the left side
              ctx.fillText(enemy.health, enemy.x + enemy.frameWidth + enemy.frameWidth / 2, enemy.y + enemy.frameHeight / 2);
            } else {
              // Default (right-facing) position for health text
          ...
          if(typeof score=='number' && score>HI){HI=score;}
          score=0;
          if(keys['r']){
            console.clear();
    ...
    let spawnLoopActive=true;
    setInterval(() => {

      if(isPause && !isGame || !spawnLoopActive) {
        return;
      }
      ...
      if (!currentWave.isFinished && waveKills>=waves[curWave].enemyCount) {
        ...
        if (currentWave.next) {
          ...
          console.log(`kills till now ${kills}`);
          waveKills=0;
          //Heal
